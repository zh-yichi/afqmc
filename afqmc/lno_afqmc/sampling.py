...
class sampler_pt2(sampler):
    ...
    def block_sample(
        ...
        e0orb_sp = t1_sp * e0orb_sp
        e1orb_sp = t1_sp * e1orb_sp
        t2orb_sp = t1_sp * t2orb_sp
        e0bar_sp = t1_sp * e0bar_sp

        wt    = jnp.sum(weights)
        eg    = jnp.sum(eg_sp * weights) / wt
        t1    = jnp.sum(t1_sp * weights) / wt
        t2orb = jnp.sum(t2orb_sp * weights) / wt
        e0orb = jnp.sum(e0orb_sp * weights) / wt
        e1orb = jnp.sum(e1orb_sp * weights) / wt
        e0bar = jnp.sum(e0bar_sp * weights) / wt
        ...
        return prop_data, (wt, eg, t1, t2orb, e0orb, e1orb, t2orb, e0bar)
